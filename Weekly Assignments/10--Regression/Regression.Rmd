...
###Univariate Analysis
(There are no categorical variables of interest. The numeric variables could be cut into intervals to force a categotical study, but this results in information loss. In lieu of that, here are three numeric graphs/analyses.)
```{r, echo=FALSE}
...
```{r}
web.capq<- (as.integer(cut(DF.c.2011$web.cap, quantile(DF.c.2011$web.cap, probs=0:4/4, na.rm=T), include.lowest=TRUE)))
...
###Chi-Square
We can also run a chi square test, dividing one of the variables into 2 levels. Say, "low internet use" and "high internet use"
...
chisq.test(T1)
```
Again, there is a clear difference. If we run a chi square test on each pair of two (a total of 6 tests), at an alpha of .05/6=.008, we find that all are significant
...
###Moderation
The possible moderator of interest is web access. This is sort of a rephrasing of what the purpose of study isfor this variable: Does accass to internet change the existing relationship between GDP and pursuit of education?
...
```
The intuitive answer in this case is not quite true. The p-value of the test is found under "Pr(>|t|)". If we are testing at alpha=.05, the interaction term is not significant.
...
summary(logit)
```
The resulting p-values would suggest that GDP is certainly related to web access, but we can't  determine that per-capita college enrollment is. 